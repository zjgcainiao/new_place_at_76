...
class InternalUserMiddleware:
    ...
    def __init__(self, get_response):
        ...
        self.customer_login_firebase_url=reverse('customer_users:customer_user_login_firebaseauth')
        self.customer_register_firebase_url=reverse('customer_users:customer_user_register_firebaseauth')
        self.customer_user_profile = reverse('customer_users:customer_user_profile')
        self.verify_firebase_token = reverse('firebase_auth_app:verify_firebase_token')
    ...
    def __call__(self, request):
        ...
        if request.path in [self.customer_login_url,self.customer_logout_url,
                            self.customer_login_firebase_url,self.customer_register_firebase_url,
        ...
        if current_app in self.PROTECTED_CUSTOMER_APPS:
            logger.info(f'  {current_app} is an protected app. Using customized rules...')
            ...
            elif not request.user.is_authenticated and uid and exp > time.time():
                logger.info('Firebase user is authenticated. Proceed to the next middleware...')
                return self.get_response(request)
            ...
            #     return redirect(self.customer_login_firebase_url)
                ...
                logger.info('CustomerUser is authenticated. Proceed to the next middleware...')
                return self.get_response(request)

            else:
                messages.error(request, f'Error! Please log in again.')
                return redirect(self.customer_login_url)
