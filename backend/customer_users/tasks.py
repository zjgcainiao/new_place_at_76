...
def create_customer_user_from_firebase_auth(firebase_user_id, password):
    ...
    try:
        customer_user = CustomerUser.objects.get(
            cust_user_email=firebase_user.firebase_user_email)  # try to get the customer_user by
    except ObjectDoesNotExist:
        customer_user = CustomerUser.objects.create_user(
            email=firebase_user.firebase_user_email, password=password)
...
def create_firebase_auth_user(uid, email, display_name):
    try:
        ...
        raise ValueError(
            'Cannot create a new FirebaseUser instance. There might be an existing user with the same uid or email.')
