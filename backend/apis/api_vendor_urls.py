...
def construct_car_seat_inspector_location_url(base_url, pattern_type, **kwargs):
    ...
    if pattern_type == 'zip':
        ...
        if all(key in kwargs for key in ['lat', 'long', 'miles']):
            ...
            raise ValueError(
                "Missing one or more of the required arguments (lat, long, miles) for pattern type 'latlong'")
    else:
