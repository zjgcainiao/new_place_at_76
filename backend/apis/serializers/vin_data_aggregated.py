...
from apis.utilities import database_sync_to_async, fetch_and_save_nhtsa_decoded_vin, \
    fetch_and_save_nhtsa_recalls, fetch_and_save_nhtsa_safety_rating, fetch_and_save_nhtsa_vehicle_id
...
class VinDataAggregatedSerializer(serializers.Serializer):
    ...
    def get_repair_history(self, obj):
        ...
        repair_orders = RepairOrdersNewSQL02Model.objects.filter(
            repair_order_vehicle__VIN_number=vin)

        repair_history_data = RepairOrderSerializer(
            repair_orders, many=True).data
        return repair_history_data

    def get_nhtsa_decoded_vin_info(self, obj):
        vin = obj['vin']
        decoded_vin = NhtsaDecodedVin.objects.filter(vin=vin)
        nhtsa_decoded_vin = NhtsaDecodedVinSerializer(
            decoded_vin, many=True).data
    ...
    def get_vin_nhtsa_snapshot_info(self, obj):
        ...
        vin_nhtsa_snapshot_data = VinNhtsaApiSnapshotsSerializer(
            vin_nhtsa_snapshot, many=True).data
