...
class VinDataAggregatedViewSet(viewsets.ViewSet):
    ...
    def list(self,request):
        ...
        # safety_ratings = NhtsaSafetyRatingSerializer(NhtsaSafetyRating.objects.all(), many=True).data
        # recall_info = RecallSerializer(NhtsaRecall.objects.all(), many=True).data
        # repair_orders = RepairOrderSerializer(RepairOrdersNewSQL02Model.objects.all(), many=True).data
        # nhtsa_decoded_vin = NhtsaDecodedVinSerializer(NhtsaDecodedVin.objects.all(), many=True).data
        
        # aggregated_data = {

        #     'safety_ratings': NhtsaSafetyRatingSerializer(NhtsaSafetyRating.objects.all(), many=True).data,
        #     'recall_info': RecallSerializer(NhtsaRecall.objects.all(), many=True).data,
        #     'repair_history': RepairOrderSerializer(RepairOrdersNewSQL02Model.objects.all(), many=True).data,
        #     'nhtsa_decoded_vin_info': NhtsaDecodedVinSerializer(NhtsaDecodedVin.objects.all(), many=True).data,
        #     'vin_nhtsa_snapshot_info': VinNhtsaApiSnapshotsSerializer(VinNhtsaApiSnapshots.objects.all(), many=True).data,
        # }     
    ...
    def retrieve(self, request, pk=None):
        ...
        repair_orders = RepairOrdersNewSQL02Model.objects.filter(repair_order_vehicle__VIN_number=vin).all
