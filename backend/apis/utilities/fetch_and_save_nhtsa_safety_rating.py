...
async def fetch_and_save_nhtsa_safety_rating(vin):
    ...
    safe_rating_exists = await database_sync_to_async(
        lambda: NhtsaSafetyRating.objects.filter(vin=vin).exists()
    )()

    if safe_rating_exists:
        nhtsa_safety_rating_queryset = await database_sync_to_async(
            lambda: NhtsaSafetyRating.objects.filter(vin=vin).all()
        )()

        # Convert the queryset to a list of objects
        nhtsa_safety_rating_list = await database_sync_to_async(list)(nhtsa_safety_rating_queryset)

        return nhtsa_safety_rating_list, [], None, "Existing record found. "
    # Check if there's an existing record in the NhtsaDecodedVin model
    decoded_vin_exists = await database_sync_to_async(NhtsaDecodedVin.objects.filter(vin=vin).exists)()
    decoded_vin = None
    if not decoded_vin_exists:
    ...
    model_year = decoded_vin.results['ModelYear'].lower()
    ...
    if vehicle_ids_exsits:
        ...
        if nhtsa_vehicle_ids:
            # retuns a tuple of nhtsa_vehicle_id_list and created_list,count and message
            nhtsa_vehicle_ids = nhtsa_vehicle_ids[0]
        ...
        try:
            ...
            if isinstance(results, dict):
                ...
                if isinstance(results, dict):
                    ...
                    nhtsa_safety_rating_list.append(
                        nhtsa_safety_rating)
